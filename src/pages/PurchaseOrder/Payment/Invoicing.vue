	...
	<view class="content">
		...
		<view class="detail">
			<view class="bianju">
				...
				<view class="name">
					<view>收款公司<span style="color: red; margin-left: 8rpx">*</span></view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			PaySubmit(Data) {
				uni.request({
					url: this.url + "/WebApiInterface/Collection/Invoicing",
					method: "POST",
					data: Data,
					success: (res) => {
			...
			submit() {
				...
				var list = {
					"voucherId": this.$store.collectionid, //账单表ID
					"ticketFiles": Files, //支付凭证
					"companyId": this.suppId,
					"userId":uni.getStorageSync('userid')
				}
				console.log(list, 'list')
					if (this.supptype == '请选择' | !list.ticketFiles) {
						uni.showToast({
							icon: "none",
