		...
		<view class="record" v-if="click==2&&payVoucher.length>0">
			...
			<view class="list" v-for="(item,index) in payVoucher" :key="index">
				<view class="subhead" v-if="item.F_PayMethod==2">
					<view class="name">
						付款账号：
						<text>{{item.F_PayBankDeposit}}/{{item.F_PayAccount}}/{{item.F_PayBankDeposit}}</text>
					</view>
				</view>
				<view class="subhead" v-if="item.F_PayMethod==2">
					<view class="name">
	...
	export default {
		...
		methods: {
			...
			cancell(type) {
				...
				this.data = {
					"id": this.$store.collectionid, // [必填]销售单主键ID
			...
			submit(data){
				...
				uni.request({
					url: this.url + '/WebApiInterface/Collection/Approval',
					method: 'POST',
					data: data,
					success: (res) => {
