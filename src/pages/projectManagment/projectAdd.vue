<template>
	<view class="page">
			...
			<view class="promid">
				...
				<view class="probom">
					...
					<view class="lis">
						...
						<input type="number" v-model="projectData.F_ProScale" placeholder="请输入项目规模"
							placeholder-class="placeinput" onkeyup="value=value.replace(/^(0+)|[^\d]+/g,'')"
							maxlength="11" @blur="F_ProScale()"></input>
					</view>
					<view class=" lis">
					...
					<view class="lis">
						...
						<!-- @click="remarkmap" -->
						<input @click="remarkmap" v-model="projectData.F_Remark2" placeholder="请输入坐标点"
					...
					<view class="lis">
						...
						<input v-model="projectData.F_DockPhone" placeholder="请输入对接电话" placeholder-class="placeinput"
							@blur="blurphone(projectData.F_DockPhone)" maxlength="11" />
					</view>
					<view class="lis">
						<text>详细地址</text>
					...
					<view class="list">
						<text>项目介绍</text>
						<textarea v-model="projectData.F_Description" placeholder="请输入项目介绍"
		...
		<view class="bombtno">
			...
			<button type="primary" @click="saleIsSubmit()">提交</button>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			// }
			// if (options.remark) {
			// 	this.remarkAdd = options.remark
			// 	this.projectData.F_Remark2 = options.remark
			// 	console.log(this.remarkAdd, 'remarkAdd')
			// }
			this.proDep()
			this.suppDep()
			this.manDep()
		...
		methods: {
			...
			clearNoNum(num) {
				...
				this.chulinum = Number(num)
				// console.log(Number(num), "最后");
				// console.log(this.chulinum, "最后");
				return this.chulinum;
			},
			F_ProScale() {
				this.clearNoNum(this.projectData.F_ProScale)
				this.projectData.F_ProScale = this.chulinum
			},
			...
			userDep(page, requestMore1) {
				...
				uni.request({
					...
					success: (res) => {
						...
						console.log(this.userData, '项目成员')
					}
				})
			},
			// 关闭窗口
			...
			saleIsSubmit() {
				let Form = {
					...
					F_ProIntroduction: this.projectData.F_Description, //项目介绍             
					// "F_Description": this.projectData.F_Description, //说明 
					F_CreatorUserId: uni.getStorageSync('userid'), //用户ID
					F_Remark: this.userid.join(), //项目成员ID 多个,号隔开
					F_Remark2: this.projectData.F_Remark2 //坐标点
				}
					...
					uni.request({
						...
						success: (res) => {
							if (res.data.StatusCode == 200) {
								...
								this.submitId = res.data.Data.F_Id
								console.log(res.data.Data.F_Id)
								this.submit()
							}
						}
					})
