...
<script>
import uCharts from '@/static/u-charts/u-charts.js'; //可以优化放全局 uCharts ==>使用全局
...
export default {
	...
	mounted() {
		let defaultOption = {
			...
			padding: [35, 15, 35, 15], //画布填充边距，顺序为上右下左，同css，但必须4位
			colors:['#2798FD','#FFAA00'],
			...
			fontSize: 9, //全局默认字体大小（可选，单位为px，默认13px）高分屏不必乘像素比，自动根据pixelRatio计算
			...
			xAxis: {
				...
				itemCount: 6, //X轴可见区域数据数量（即X轴数据密度），配合拖拽滚动使用（即仅在启用enableScroll时有效）
			...
			yAxis: {
				...
				format: val => {
					...
					if (type == 'number') {
						...
						let newName = name || '';
						return `${(val * 2).toFixed(fixed)}${name}${newName}`;
					} else {
			...
			extra: {
				column: {
					//参考官网配置
					type: 'group',
					width: (this.cWidth * 0.2) / this.dataAs[this.labelKey].length
				}
			}
		};
